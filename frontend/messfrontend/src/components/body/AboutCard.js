const AboutCard = () => {
    return (
        <>
            <div>
                <div className="Main">
                    <div className="container box box1 zIndBack">
                        <div className=" headingBox">
                            <div className="headingLarge largeFont zIndBack">
                                About
                            </div>
                            <div className="headingSmall smallFont zIndFront">
                                About us
                            </div>
                        </div>
                        <div className="container aboutDetails">
                            Lorem ipsum dolor sit amet consectetur adipisicing elit. Nisi quae sint atque similique sunt maiores cum repellendus odit! Voluptatum ab adipisci facilis amet quasi minima dolorem officia asperiores quaerat libero repellat suscipit labore, magni placeat quis quas quam eos ipsam reiciendis ratione quibusdam voluptate voluptatibus eveniet! Nisi eaque cupiditate accusamus!
                        </div>

                    </div>
                    <div className="container box box2 zIndFront">Logo</div>
                </div>
            </div>

        </>
    )
}
export default AboutCard;